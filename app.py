...
SOFASCORE_AUTO_EXTRACTOR_VERSION = "v23.35.0-sofascore-auto-extractor"
SOFASCORE_API_BASES = [
...
def fetch_sofascore_tennis_schedule(fecha_iso):
    """Devuelve partidos del día desde SofaScore. Best-effort: si falla, se mantiene pegado manual."""
    last_error = ""
    for base in SOFASCORE_API_BASES:
        url = f"{base}/sport/tennis/scheduled-events/{fecha_iso}"
        try:
            r = requests.get(url, headers=_sofa_headers(), timeout=12)
            if r.status_code != 200:
                last_error = f"{r.status_code} en {base}"
                continue
            data = r.json()
            ...
            for ev in events:
                ...
                if m:
                    matches.append(m)
            # Orden por hora + torneo para que el Excel salga limpio.
            matches = sorted(matches, key=lambda x: (str(x.get("time", "")), str(x.get("torneo", "")), str(x.get("p1_raw", ""))))
            return {"ok": True, "matches": matches, "url": url, "error": ""}
        except Exception as e:
            last_error = str(e)
    return {"ok": False, "matches": [], "url": "", "error": last_error or "No se pudo conectar con SofaScore"}
