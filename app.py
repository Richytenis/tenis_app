...
# =========================================================
st.set_page_config(page_title="Tennis IA Predictor Ultra v4.8", page_icon="🎾", layout="wide")
...
# =========================================================
# MOTOR DE CÁLCULO VERSIÓN 4.8 (ELITE ADJUSTMENT)
# =========================================================
def obtener_hold_rate(e1, e2, circuito_ui, superficie, nivel_torneo):
    # --- LÓGICA ATP (SIN CAMBIOS) ---
    if circuito_ui == "ATP":
        base = 0.81
    ...
    # --- LÓGICA WTA (DIVISOR MAESTRO 3500) ---
    elif circuito_ui == "WTA":
        base = 0.74 # Subimos la base para evitar sets de 25 juegos
        if superficie == "Clay": base -= 0.04
        # Divisor masivo: reduce el impacto de la diferencia de Elo
        divisor = 3500 
        min_h = 0.55 # Las profesionales mantienen el saque más de lo que el Elo sugiere
...
def sim_set(p1_h, p2_h):
    ...
    sacador = 1
    while True:
        # Boost de Dominio (6%): Para forzar cierres de set rápidos 6-2 / 6-3
...
# INTERFAZ
# =========================================================
st.title("🎾 Tennis IA Predictor Ultra v4.8")
...
if not jugadores_filtrados:
    ...
    c1, c2 = st.columns(2)
    with c1: j1_n = st.selectbox("Jugador 1", jugadores_filtrados)
    with c2: j2_n = st.selectbox("Jugador 2", jugadores_filtrados, index=min(1, len(jugadores_filtrados)-1))
    ...
    if st.button("🚀 ANALIZAR", use_container_width=True):
        ...
        st.info(f"**Final Analysis:** Promedio Juegos: {sum(juegos)/n_sims:.1f} | Saque: {h1:.1%} / {h2:.1%}")